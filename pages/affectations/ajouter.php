...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_enseignant) || empty($id_matiere) || empty($id_classe) || empty($annee_scolaire)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO affectation (id_enseignant, id_matiere, id_classe, annee_scolaire)
                               VALUES (:ens, :mat, :cls, :annee)");
        $stmt->execute([':ens'=>$id_enseignant, ':mat'=>$id_matiere, ':cls'=>$id_classe, ':annee'=>$annee_scolaire]);
        header("Location: liste.php?msg=Affectation ajoutée avec succès !");
        exit;
    }
}
    ...
    <div class="form-col">
        <div class="card">
            <div class="card-header primary">
                <h5 class="">➕ Ajouter une affectation</h5>
            </div>
            <div class="card-body">
                <?php if ($erreur): ?>
